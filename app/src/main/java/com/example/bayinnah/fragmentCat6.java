...
public class fragmentCat6 extends Fragment {
    ...
    private void loadNextQuestion() {
        ...
        if (questionCounter < totalQuestions) { // Check if there are more questions to load
            quizCursor = quizDb.rawQuery("SELECT question, option1, option2, option3, option4, image,answer,stage FROM quiz_questions WHERE stage=6 ORDER BY RANDOM() LIMIT 10", null);
            ...
            if(score1<score) {
                String strSQL = "UPDATE level_score SET score=" + score + " WHERE stage =6 ";
